...
# Load report
data <- read.csv("/Users/cerpelloni/Desktop/GitHub/VisualBraille_data/code/stats/split_unvariateReport.txt")
...
stats_data <- data %>% group_by(mask, condition, script, contrast, cluster, group) %>% 
...
ggplot(stats_data, aes(x = condition, y = mean_peak)) + 
  ...
  # aesthetics = c("color", "fill") allows to do both at the same time
  # cool but dots on bars are not visible
  geom_point(data = data, 
             aes(x = reorder(condition, cluster),
                 y = peak),
             position = position_jitter(w = 0.3, h = 0.01),
...
ggsave("figures/cond-splitVWFA_univariate_peak-activation.png", width = 3000, height = 1800, dpi = 320, units = "px")
