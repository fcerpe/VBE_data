...
# Load slopes report
slopes <- read.csv("/Users/cerpelloni/Desktop/GitHub/VisualBraille_data/code/ppi/slopesReport.txt")
...
slopes$condition <- ifelse(slopes$group == "expert", 
...
allPoints <- read.csv("/Users/cerpelloni/Desktop/GitHub/VisualBraille_data/code/ppi/datapointsPPI.txt")
...
# Bar plot of average correlation 
ggplot(stats_corr, aes(x = condition, y = mean_corr)) + 
  geom_col(aes(x = condition, y = mean_corr, fill = cluster)) +
  geom_errorbar(aes(ymin = mean_corr - se_corr, ymax = mean_corr + se_corr), width = 0) +
  scale_fill_manual(name = "script x group",
                    limits = c("french_expert",   "braille_expert",   "french_control",   "braille_control"),
                    values = c("#69B5A2",         "#FF9E4A",          "#699ae5",          "#da5F49"),
                    labels = c("french - expert", "braille - expert", "french - control", "braille - control")) +
  ...
  scale_x_discrete(limits = c("bw_ctr","bw_exp","fw_ctr","fw_exp","sbw_ctr","sbw_exp","sfw_ctr","sfw_exp"),
                   labels = c("BW","BW","FW","FW","SBW","SBW","SFW","SFW")) +
  labs(x = "Stimulus condition", y = "VWFA-LPosTemp correlation", title = "Average correlation in PPI: VWFA x LPosTEmp")

ggsave("figures/cond-PPI_areas-VWFA-LPosTemp_mean-correlation.png", width = 3000, height = 1800, dpi = 320, units = "px")
