...
%   opt = chooseMaskNEW(opt);
  ...
  directionNb = [1 2 1 2 1 2 ];
  directionName = {'vertical', 'horizontal','vertical','horizontal','vertical', 'horizontal'};
...
%         };
    ...
    opt.maskPath = fullfile(fileparts(mfilename('fullpath')), '..','..', '..','derivatives' ,'cpp_roi_sphere',strcat('radiusNb-',num2str(radiusNb)),strcat('sub-',subID));
    ...
    for iImage = 1:length(opt.mvpa.map4D)
      ...
      for iMask = 1:length(opt.maskName)
        ...
        image=fullfile(fileparts(mfilename('fullpath')),'..', '..','..','derivatives','mvpaVol',strcat('sub-',subID),imageName);
