function [maskVoxel, opt] = mvpa_masks_checkSize(opt)
% Runs a quick decoding to figure out the sizes of all the masks
    ...
    for iImage = 1:length(opt.mvpa.map4D)
        ...
        for iMask = 1:length(subMasks)
            ...
            if ismember(subMasks{iMask}, availableMasks)
                ...
                opt.report = vertcat(opt.report,{reportSize, reportMask});
            end
        end
