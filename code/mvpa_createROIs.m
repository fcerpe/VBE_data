...
%
% Steps:
% 1 - open bspmview and manually get the peak coordinates in each subject for each ROI
...
% -65 -41.8 5.40 
% LOC-L [-44.2 -78.2  -7.6]
% PFS-L [-39.0 -62.6 -12.8]
% LOC-R [ 36.4 -83.4 -10.2]
% PFS-R [ 36.4 -52.2 -12.8]
%
...
% SUB-004
mni{4}(1, 1:3) = [NaN NaN NaN];  % VWFA
mni{4}(2, 1:3) = [NaN NaN NaN];  % LOC LEFT
mni{4}(3, 1:3) = [NaN NaN NaN];  % PFS LEFT
mni{4}(4, 1:3) = [NaN NaN NaN];  % LOC RIGHT
mni{4}(5, 1:3) = [NaN NaN NaN];  % PFS RIGHT
% mni{4}(6, 1:3) = [0 0 0];  % pSTG LEFT

% SUB-005
mni{5}(1, 1:3) = [-6.500000e+01, -4.180000e+01,  0.540000e+01];  % VWFA
mni{5}(2, 1:3) = [-4.420000e+01, -7.820000e+01, -0.760000e+01];  % LOC LEFT
mni{5}(3, 1:3) = [-3.900000e+01, -6.260000e+01, -1.280000e+01];  % PFS LEFT
mni{5}(4, 1:3) = [ 3.640000e+01, -8.340000e+01, -1.020000e+01];  % LOC RIGHT
mni{5}(5, 1:3) = [ 3.640000e+01, -5.220000e+01, -1.280000e+01];  % PFS RIGHT
% mni{5}(6, 1:3) = [0 0 0];  % pSTG LEFT


ROI_name_list = {'VWFA-Left', 'LOC-Left', 'PFS-Left', 'LOC-Right', 'PFS-Right'}; 
% ,'pSTG-Left' if needed later

save('visROI_mni_coordinates-s5variant.mat', 'ROI_name_list', 'mni');
...
% for which subjects are we getting the ROIs?
subjectsList = [2,3,5];

for iSub = 1:length(subjectsList)

    % Get subject name, in our single ROI is the row with the coordinates
    subName = ['sub-00', num2str(subjectsList(iSub))];

    % If there is actually something there
    if ~isnan(mni{subjectsList(iSub)})

        for iReg = 1:size(mni{subjectsList(iSub)}, 1) % for each region this subject has
            % Get the center
            ROI_center = mni{subjectsList(iSub)}(iReg, :);
