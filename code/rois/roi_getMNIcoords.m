function mni = roi_getMNIcoords(opt)
...
% e.g. mni{1}= [-46 64 4]
mni = cell(length(opt.subjects),1);

% Based on space: coordinates seem to differ
switch opt.space{1}

    % bidspm processing
    case 'IXI549Space'

        for i = 1:length(opt.subjects)

            switch opt.subjects{i}
                ...
                case '006'
                    ...
                    mni{i}(3, 1:3) = [-39.00 -75.60 -15.40]; % LO LEFT
    ...
    case 'MNI152NLin2009cAsym'
        ...
        for i = 1:length(opt.subjects)
            ...
            switch opt.subjects{i}
                case '006'
                    mni{i}(1, 1:3) = [-49.36 -61.79 -08.30]; mni{i}(2, 1:3) = [-49.40 -62.60 -12.80]; 
                    mni{i}(3, 1:3) = [-52.00 -65.20 -00.20]; mni{i}(4, 1:3) = [ 39.00 -67.80 -10.20]; 

                case '007'
                    mni{i}(1, 1:3) = [-52.00 -49.60 -18.00]; mni{i}(2, 1:3) = [-52.00 -49.60 -18.00]; 
                    mni{i}(3, 1:3) = [-44.20 -65.20 -12.80]; mni{i}(4, 1:3) = [ 46.80 -78.20  00.20]; 

                case '008'
                    mni{i}(1, 1:3) = [-52.00 -54.80 -18.00]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-41.60 -78.20 -02.40]; mni{i}(4, 1:3) = [ 44.20 -78.20 -07.60]; 

                case '009'
                    mni{i}(1, 1:3) = [-44.20 -62.60 -12.80]; mni{i}(2, 1:3) = [-41.60 -62.60 -10.20]; 
                    mni{i}(3, 1:3) = [-49.40 -78.20 -02.40]; mni{i}(4, 1:3) = [ 36.40 -70.40 -12.80]; 

                case '010'
                    mni{i}(1, 1:3) = [-44.20 -62.60 -12.80]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-49.40 -75.60  00.20]; mni{i}(4, 1:3) = [ 49.40 -65.20 -05.00]; 

                case '011'
                    mni{i}(1, 1:3) = [-49.40 -60.00 -10.20]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-39.00 -73.00 -10.20]; mni{i}(4, 1:3) = [ 44.20 -65.20 -10.20]; 

                case '012'
                    mni{i}(1, 1:3) = [-44.20 -60.00 -15.40]; mni{i}(2, 1:3) = [-44.20 -57.40 -18.00]; 
                    mni{i}(3, 1:3) = [-49.40 -75.60 -05.00]; mni{i}(4, 1:3) = [ 49.40 -73.00 -10.20]; 

                case '013'
                    mni{i}(1, 1:3) = [-44.20 -60.00 -12.80]; mni{i}(2, 1:3) = [-46.80 -62.60 -15.40]; 
                    mni{i}(3, 1:3) = [-49.40 -78.20 -02.40]; mni{i}(4, 1:3) = [ 49.40 -67.80 -12.80]; 

                case '018'
                    mni{i}(1, 1:3) = [-46.80 -54.80 -07.60]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-46.80 -73.00 -05.00]; mni{i}(4, 1:3) = [ 44.20 -70.40 -10.20]; 

                case '019'
                    mni{i}(1, 1:3) = [-46.80 -47.00 -20.60]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-41.60 -78.20 -05.00]; mni{i}(4, 1:3) = [ 44.20 -73.00 -07.60]; 

                case '020'
                    mni{i}(1, 1:3) = [-44.20 -57.40 -20.60]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-49.40 -78.20 -05.00]; mni{i}(4, 1:3) = [ 44.20 -65.20 -12.80]; 

                case '021'
                    mni{i}(1, 1:3) = [-46.80 -49.60 -23.20]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-46.80 -65.20 -12.80]; mni{i}(4, 1:3) = [ 49.40 -67.80 -07.60]; 

                case '022'
                    mni{i}(1, 1:3) = [-47.70 -64.90 -18.00]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-42.50 -75.30 -10.20]; mni{i}(4, 1:3) = [ 49.20 -67.50 -10.20]; 

                case '023'
                    mni{i}(1, 1:3) = [-46.80 -49.60 -23.20]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [   NaN    NaN    NaN]; mni{i}(4, 1:3) = [ 39.00 -67.80 -05.00]; 

                case '024' % Control subject, Drawings v. scrambled contrast did not show LOC
                    mni{i}(1, 1:3) = [-41.60 -52.20 -10.20]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [   NaN    NaN    NaN]; mni{i}(4, 1:3) = [   NaN    NaN    NaN]; 

                case '026'
                    mni{i}(1, 1:3) = [-52.00 -57.40 -15.40]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-49.40 -73.00 -10.20]; mni{i}(4, 1:3) = [ 44.20 -67.80 -12.80]; 

                case '027'
                    mni{i}(1, 1:3) = [-54.60 -57.40 -15.40]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-52.00 -75.60  00.20]; mni{i}(4, 1:3) = [ 39.00 -75.60 -05.00]; 

                case '028'
                    mni{i}(1, 1:3) = [-54.60 -49.60 -12.80]; mni{i}(2, 1:3) = [   NaN    NaN    NaN]; 
                    mni{i}(3, 1:3) = [-46.80 -73.00 -12.80]; mni{i}(4, 1:3) = [ 39.00 -73.00 -10.20]; 
